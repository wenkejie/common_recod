...
(function($, _undefined) {
	...
	function Core(widget) {
		...
		this.init = function() {
			this.selectDiv = $(
					"<div class='fui-form-cell fui-select'><div class='fui-select-cnt'><div class='fui-select-input'></div><s class='fui-form-icon icon-select-arrow'></s><div class='fui-drop-down-con none'><ul class='fui-select-list'></ul></div></div></div>")
		...
		this.setByKey = function(key) {
			...
			if (item) {
				...
				this.selectDiv.find("div.fui-select-input").html(this.selectDiv.find("li[key=" + key + "]").html());
			} else {
				widget.element.val("");
				this.selectDiv.find("div.fui-select-input").html("");
			}
			this.select(key);
		};
		...
		this.scrollTop = function() {
			...
			if (widget.options.size && this.selectDiv.find("li").size() > widget.options.size) {
				this.selectDiv.find("div.fui-drop-down-con").scrollTop(
						this.selectDiv.find("li").index(li) * 22 + 11 + 5 - this.selectDiv.find("div.fui-drop-down-con").height() / 2);

				this.selectDiv.find("div.fui-drop-down-con li").css("padding-right", "32px");
			}
		};
		...
		this.open = function() {
			if (!this.isOpen) {
				...
				this.selectDiv.find("div.fui-drop-down-con").show();
				this.scrollTop();
				$(document.body).bind("keyup.fselect", function(event) {
					var li;
		...
		this.close = function() {
			if (this.isOpen) {
				...
				this.selectDiv.find("div.fui-drop-down-con").hide();
				$(document.body).unbind("keyup.fselect");
		...
		this.size = function(size) {
			if (size && this.selectDiv.find("li").size() > size) {
				this.selectDiv.find("div.fui-drop-down-con").css("height", 22 * size + 10);
			} else {
				this.selectDiv.find("div.fui-drop-down-con").css("height", "auto");
			}
		};
		this.width = function(width) {
			...
			this.selectDiv.find("div.fui-drop-down-con").css("min-width", this.selectDiv.outerWidth() - 2);
		};
		this.itemWidth = function(width) {
			if (width) {
				this.selectDiv.find("div.fui-drop-down-con").width(width - 2);
			} else {
				this.selectDiv.find("div.fui-drop-down-con").width("auto");
			}
		};
