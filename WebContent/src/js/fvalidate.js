 ...
 * @description 页面元素验证 需要添加页面级的滚动区域元素[.fui-scroll,.fui-scroll-x,.fui-scroll-y]，该元素带滚动属性且具有样式[position:relative]
 * 
	...
	validate : function(options) {
		...
		self.each(function(index,element){
			...
			if($element.is("[validate]")){ 
				...
				var _elements = $element.find("[validate]");
...
//			}
			if (msg) {
				...
				self._parent.addClass("fui-form-cell-erro").unbind("mouseenter.susdValidate").bind("mouseenter.susdValidate", function() {
				...
				self._parent.removeClass("fui-form-cell-erro").unbind("mouseenter.susdValidate");
			}
		} else {
			/* 验证格式串格式错误 */
			$(document.body).html(
	...
	_vInputSelect : function(m, required, type, min, max, decimal, minValue, maxValue, extVal) {
		...
		}else if(!msg && type == "s"){//modify by 曹金彦,2014-7-22,回车算作2个字符计算
			size = val.replace(/\n/g,"##").length;
		}
...
//		return msg;
	...
	_removeErrorInfoBox : function(){
		...
		if(!box){
			var _fui_scroll_cnt = self.data("container");
			if(!_fui_scroll_cnt){
				_fui_scroll_cnt = self.parents(".fui-scroll,.fui-scroll-x,.fui-scroll-y");
				if($.fn.defaults.container){
					_fui_scroll_cnt = $.fn.defaults.container;
				}else{
					$.fn.defaults.container = _fui_scroll_cnt;
				}
			}
			if(_fui_scroll_cnt.size()>0){
				_fui_scroll_cnt = _fui_scroll_cnt.eq(0);
			}else{
				/* 验证格式串格式错误 */
				$(document.body).html(
						"<textarea style='width:100%;height:500px;font-size:50px;'>error:\n 缺少页面级滚动元素 \n $(\".fui-scroll,.fui-scroll-x,.fui-scroll-y\")</textarea>");
				throw Error;
			}
			box =  $.fn.defaults.container.data("errorInfoBox");//._errorInfoBox;
		}
		if (box) {
			self._parent.removeClass("fui-form-cell-erro").unbind("mouseenter.susdValidate").unbind("mouseleave.susdValidate");
		}
	},
	_getErrorInfoBox : function(msg, top, left) {
		...
		if(!_fui_scroll_cnt){
			...
			self.data("container",_fui_scroll_cnt);
		}
		if(_fui_scroll_cnt.size()>0){
			_fui_scroll_cnt = _fui_scroll_cnt.eq(0);
		}else{
		...
		if (!box) {
			box = $("<div class=\"fui-tips fui-tips-red fui-tips-hasClose fui-tips-oneLine none\"></div>");
			//self.after(box);
			_fui_scroll_cnt.append(box);
			self.data("errorInfoBox", box);
			//$.fn.defaults._errorInfoBox = box;
			$.fn.defaults.container.data("errorInfoBox", box);
		}
		if (msg) {
			box.html("<div class=\"fui-tips-arrow top\">◆<em>◆</em></div><s class=\"icon-info-red\"></s>"+msg);
			var dLeft = left || Math.max(0, self._parent.position().left);
			var _fui_scroll_cnt_width = _fui_scroll_cnt.outerWidth();
			if((dLeft + box.width())>(_fui_scroll_cnt.position().left + _fui_scroll_cnt_width)){
				dLeft = dLeft + self._parent.outerWidth() - box.outerWidth();
				var _fui_tips_arrow = box.find(".fui-tips-arrow");
				_fui_tips_arrow.css("left",(parseInt(_fui_tips_arrow.css("left")) + box.width() - 8)+"px");
			}
			//console.log(box.offset().right);
			
			var dTop = top || Math.max(0, self._parent.position().top + self._parent.outerHeight() + 6);
			var _scrollTop = _fui_scroll_cnt.scrollTop();
			if(_scrollTop>0){
				dTop = dTop + _scrollTop;
			}
			if((dTop + box.height()) > _fui_scroll_cnt.height()){
				var _top = 0;
				if(self.is("textarea")){
				...
				if(_top>0){
					box.find(".fui-tips-arrow").removeClass("top").addClass("bottom");
					dTop = _top; 
				}
			}
			box.css( {
